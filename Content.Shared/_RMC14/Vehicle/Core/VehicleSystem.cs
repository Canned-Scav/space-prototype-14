...
public sealed class VehicleSystem : EntitySystem
{
    ...
    // rmc-edit: removed Content.Shared.Vehicle.VehicleSystem _vehicles (unused in RMC VehicleSystem)
    ...
    private bool TryEnter(Entity<VehicleEnterComponent> ent, EntityUid user, int entryIndex = -1)
    {
        ...
        // rmc-edit: removed xeno capacity check, treating all occupants as passengers
        if (!isGhost)
        {
        ...
        // rmc-edit: replaced _rmcTeleporter.HandlePulling with standard transform teleport
    ...
    private bool EnsureInterior(Entity<VehicleEnterComponent> ent, [NotNullWhen(true)] out VehicleInteriorComponent? interior)
    {
        ...
        // rmc-edit: removed interior.Xenos.Clear() - Xenos not ported
    ...
    private void CleanupInterior(EntityUid vehicle)
    {
        ...
        // rmc-edit: removed Xenos loop - Xenos not ported to Goob
        // Passengers loop above handles all occupant cleanup
    ...
    // rmc-edit: removed isXeno parameter - all occupants tracked as passengers
    ...
    {
        // rmc-edit: removed xeno tracking, only passengers now
        foreach (var passenger in new List<EntityUid>(interior.Passengers))
        {
            if (TryComp(passenger, out VehicleInteriorOccupantComponent? occupant) &&
                occupant.Vehicle == vehicle &&
    ...
    {
        // rmc-edit: skill check removed, Marines.Skills not ported
        if (args.Cancelled)
            return;
    }
    ...
    {
        // rmc-edit: removed xeno bypass, keeping only frame integrity check
