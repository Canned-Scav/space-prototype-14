...
// rmc-edit: removed using Content.Shared._RMC14.Repairable (RMCRepairableSystem replaced with standard tool quality check)
using Content.Shared.Tools.Components;
// rmc-edit: removed using Content.Shared._RMC14.Tools (no RMC tool types used)
...
// rmc-edit: replaced Content.Shared.FixedPoint with Content.Goobstation.Maths.FixedPoint
using Content.Goobstation.Maths.FixedPoint;
using Content.Shared.Explosion.EntitySystems;
// rmc-edit: removed using Content.Shared._RMC14.Xenonids
// rmc-edit: removed using Content.Shared._RMC14.Marines.Skills

namespace Content.Shared._RMC14.Vehicle;

public sealed partial class HardpointSystem : EntitySystem
{
    // rmc-edit: removed EngineerSkill constant - Marines.Skills not ported
    ...
    // rmc-edit: removed _repairable (RMCRepairableSystem) and _skills (SkillsSystem)
    ...
    private void EnsureSlots(EntityUid uid, HardpointSlotsComponent component, ItemSlotsComponent? itemSlots = null)
    {
        ...
        foreach (var slot in component.Slots)
        {
            ...
            else
            {
                ...
                // rmc-edit: removed hasSkills check - EntityWhitelist.Skills not in Goob
                // rmc-edit: removed hasMinMobSize - EntityWhitelist.MinMobSize not in Goob
    ...
    private void OnVehicleDamageModify(Entity<HardpointSlotsComponent> ent, ref DamageModifyEvent args)
    {
        ...
        // rmc-edit: args.Tool removed - DamageModifyEvent in Goob doesn't have Tool field
    ...
    private void OnHardpointExamined(Entity<HardpointIntegrityComponent> ent, ref ExaminedEvent args)
    {
        ...
        // rmc-edit: removed xeno-specific examine text - xenos see the same integrity as everyone else
    ...
    private void OnHardpointRepair(Entity<HardpointIntegrityComponent> ent, ref InteractUsingEvent args)
    {
        ...
        // rmc-edit: replaced _repairable.UseFuel with standard tool quality check
        // (RMCRepairableSystem not ported - fuel will be consumed in DoAfter)
        if (usedWelder && !_tool.HasQuality(used!, ent.Comp.RepairToolQuality))
        {
            args.Handled = true;
            return;
        }
    ...
    private void OnHardpointRepairDoAfter(Entity<HardpointIntegrityComponent> ent, ref HardpointRepairDoAfterEvent args)
    {
        ...
        if (usedWelder)
        {
            if (used == null || !_tool.HasQuality(used.Value, ent.Comp.RepairToolQuality))
                return;
        }
        ...
        bool isFrame)
    {
        ...
        // rmc-edit: removed skill multiplier (Marines.Skills not ported) - use 1.0f multiplier
        var skillMultiplier = 1.0f;
